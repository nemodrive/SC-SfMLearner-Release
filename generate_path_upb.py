...
def main():
    ...
    for seq in sequences:
        if '.txt' not in seq:
            ...
            df = pd.read_csv("results/vo/cs+k_pose_upb/{}.txt".format(args.sequence), sep=" ", header=None)
            ...
            file_name = args.dataset_dir.split('/')[-2].split('_')[0]
            ...
            for i in tqdm(range(len(pose))):
                ...
                # this tries to see if there are obstacles, based on the disparity
                ...
                # cv2.imwrite(args.segmentation_path + "labels/JPEGImages/" + seq + "_{0:06d}.jpg".format(i), show_img)
